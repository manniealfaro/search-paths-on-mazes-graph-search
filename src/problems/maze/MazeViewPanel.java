...
public class MazeViewPanel extends JPanel{
	...
	// Images
	public static final Image hamster = Toolkit.getDefaultToolkit().getImage("/Users/manuel/Projects/practice1IS/src/imgs/hamster.png");
	public static final Image hamsterFull = Toolkit.getDefaultToolkit().getImage("/Users/manuel/Projects/practice1IS/src/imgs/hamster2.png");
	public static final Image cheese = Toolkit.getDefaultToolkit().getImage("/Users/manuel/Projects/practice1IS/src/imgs/queso.png");

	//public static final Image hamster = Toolkit.getDefaultToolkit().getImage("C:/Users/José Carlos/IdeaProjects/Ass1_IS/src/imgs/hamster.png");
	//public static final Image hamsterFull = Toolkit.getDefaultToolkit().getImage("C:/Users/José Carlos/IdeaProjects/Ass1_IS/src/imgs/hamster2.png");
	//public static final Image cheese = Toolkit.getDefaultToolkit().getImage("C:/Users/José Carlos/IdeaProjects/Ass1_IS/src/imgs/queso.png");
	// Maze and status
	private Maze maze;							// Maze	
	private int[] posHamster = {0,0};			// 2 element array with x and y coordinates of the hamster
	private boolean hamsterHasEaten = false; 	// Flag that indicates if the hamster has eaten
	...
	Image scaledHamster;			// Scaled hamster1
	Image scaledHamsterFull;		// Scaled hamster1
	...
	public MazeViewPanel(Maze maze, int sizePx){
		...
		scaledHamsterFull = hamsterFull.getScaledInstance((int)(cellSizePx*0.8), (int)(cellSizePx*0.8), Image.SCALE_SMOOTH);
	...
	private void generateMazeImage(){
		...
		// Paints the cheese
		int[] posCheesePx = posToPx(maze.size-1, maze.outputX);
		posCheesePx[0] = posCheesePx[0] + (int)(cellSizePx*0.1); // To center the cheese
		posCheesePx[1] = posCheesePx[1] + (int)(cellSizePx*0.1);	
		mazeGraphics2D.drawImage(scaledCheese, posCheesePx[1], posCheesePx[0], this);
		// Paints the cheese
		for (posX=0;posX<maze.size;posX++){
			for (posY=0;posY<maze.size;posY++)
				if (maze.cells[posY][posX]==3){				
					posCheesePx = posToPx(posY, posX);
					posCheesePx[0] = posCheesePx[0] + (int)(cellSizePx*0.1); // To center the cheese
					posCheesePx[1] = posCheesePx[1] + (int)(cellSizePx*0.1);					
					// Otherwise it does not synchronize well
					try {
						Thread.sleep(200);
...
//		posHamsterPx[1] = posHamsterPx[1] + (int)(cellSizePx*0.1);
		if (hamsterHasEaten)
			graphics2D.drawImage(scaledHamsterFull, posHamsterPx[1], posHamsterPx[0], this);
		else
...
//		goalHamsterPx[1] = goalHamsterPx[1] + (int)(cellSizePx*0.1); 
	...
	/** Eats the cheese. This is for graphics only, so it is possible
	 * to make the cheese dissapear even if the hamster is somewhere else. */
	public void eat(){
		hamsterHasEaten = true;
	}
